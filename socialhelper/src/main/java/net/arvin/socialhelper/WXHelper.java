...
//            activity.registerReceiver(wxShareReceiver, new IntentFilter(SocialHelper.WX_SHARE_RECEIVER_ACTION));
    ...
    private boolean addImage(SendMessageToWX.Req req, WXMediaMessage msg, Bundle params) {
        ...
        Bitmap bitmap = null;
        if (params.containsKey(WXShareEntity.KEY_WX_IMG_BYTES)) {
            byte[] byteArray = params.getByteArray(WXShareEntity.KEY_WX_IMG_BYTES);
            imgObj = new WXImageObject(byteArray);
            //设置缩略图
//            msg.thumbData = byteArray;
        ...
        } else if (params.containsKey(WXShareEntity.KEY_WX_IMG_LOCAL)) {//分为本地文件和应用内资源图片
